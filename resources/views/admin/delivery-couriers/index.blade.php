...
@section('content')
@php
    $pageTitle = __('admin.delivery_couriers_management');
@endphp
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900">
                {{ __('admin.delivery_couriers') }}
            </h1>
            <p class="mt-1 text-sm text-gray-500">
                {{ __('admin.delivery_couriers_description') }}
            </p>
        </div>

        @can('manage delivery couriers')
        <a href="{{ route('admin.delivery-couriers.create') }}"
...
<div class="mb-6 bg-white rounded-xl shadow-sm border border-gray-100 p-4">
    <form method="GET" action="{{ route('admin.delivery-couriers.index') }}" class="flex flex-col sm:flex-row gap-4">
        {{-- Search --}}
        <div class="flex-1">
            <input type="text"
                   name="search"
                   value="{{ request('search') }}"
                   placeholder="{{ __('admin.search_couriers') }}"
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($couriers as $courier)
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            <div class="text-sm text-gray-600">
                                {{ $courier->created_at->format('Y-m-d') }}
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2
                                ...
                                @can('view delivery couriers')
                                    {{-- View --}}
                                    <a href="{{ route('admin.delivery-couriers.show', $courier) }}"
                                ...
                                @can('manage delivery couriers')
                                    {{-- Edit --}}
                                    <a href="{{ route('admin.delivery-couriers.edit', $courier) }}"
                                       class="p-2 text-gray-400 hover:text-primary
                                              rounded-lg hover:bg-gray-100
                                              transition-colors duration-150"
                                       title="{{ __('admin.edit') }}">
                                    ...
                                    {{-- Toggle Status --}}
                                    <form method="POST"
                                          action="{{ route('admin.delivery-couriers.toggle-status', $courier) }}"
                                          class="inline">
                                        @csrf
                                        <button type="submit"
                                                class="p-2 text-gray-400 hover:text-yellow-600
                                                       ...
                                                       transition-colors duration-150"
                                                title="{{ $courier->is_active ? __('admin.deactivate') : __('admin.activate') }}">
                                            @if($courier->is_active)
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    {{-- Delete --}}
                                    <button type="button"
                                            class="delete-courier-btn p-2 text-gray-400 hover:text-red-600
                                                   rounded-lg hover:bg-red-50
                                                   transition-colors duration-150"
                                            data-url="{{ route('admin.delivery-couriers.destroy', $courier) }}"
                                            data-item-name="{{ $courier->name }}"
                                            data-modal-confirm="{{ __('admin.confirm_delete_courier') }}"
                        ...
                        <td colspan="5" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-lg font-medium text-gray-900 mb-1">
                                    {{ __('admin.no_couriers_found') }}
                                </h3>
                                <p class="text-sm text-gray-500 mb-4">
                                    {{ __('admin.no_couriers_description') }}
                                </p>
                                @can('manage delivery couriers')
                                <a href="{{ route('admin.delivery-couriers.create') }}"
                                   class="inline-flex items-center gap-2 px-4 py-2
